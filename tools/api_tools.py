...
# # @mcp.tool()
# # def get_items(query: str, limit : int = 5) -> str:
# #     r = requests.get(f"https://www.loc.gov/search/?q={query}&fo=json&at=results&c={limit}").json()
# #     return r
...
#     query = query
#     collections_json = requests.get(f"https://www.loc.gov/collections/?fo=json&at=results&q={query}").json()
#     return collections_json
...
@mcp.tool()
def format_search(query: str, format: str = "photos", max_results: int = 5) -> str:
    """
    ...
    file_path = os.path.join(path, "formats_items_info.json")
    ...
    for item in results:
        ...
        item_info = {
            ...
            "description": item.get("description", ""), #item['description'][0] if item['description'][0] else "",
            "date": item.get('date', ""),
            "url": item.get("id", ""),
            "thumbnail": item.get("image_url")[0]
        }
...
@mcp.tool()
def search_items(query: str, max_results: int = 5) -> List[str]:
    """
